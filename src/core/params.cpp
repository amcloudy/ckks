                       ...
                       const ChainDesign& d)
{
    ...
    if (required_logQ > max_logQ) {
        throw std::runtime_error("Chain exceeds security bound");
    }

    std::vector<std::uint64_t> qi_list;
    qi_list.reserve(levels);

    const std::uint64_t approx_bits = d.log_scale;
    const std::uint64_t low = (approx_bits >= 63)
        ? (std::numeric_limits<std::uint64_t>::max() >> 1)
        : (1ULL << approx_bits);
...
static bool is_probable_prime(std::uint64_t n)
{
    ...
    auto check = [&](std::uint64_t a) {
        ...
        std::uint64_t x64 = (uint64_t)x;
        if (x64 == 1 || x64 == n-1) return true;
        for (uint64_t r = 1; r < s; ++r) {
            x = (x * x) % n;
            if ((uint64_t)x == n - 1) return true;
        }
        return false;
    };
...
static std::uint64_t next_prime_congruent_1_mod(std::uint64_t s,
                                                std::uint64_t mod)
{
    std::uint64_t x = s;
    if (x % mod != 1) x += (mod - (x % mod)) + 1;
    for (;;) {
        if (is_probable_prime(x)) return x;
        x += mod;
    }
